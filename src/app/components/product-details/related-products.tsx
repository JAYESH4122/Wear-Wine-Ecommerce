    ...
    <section className="py-16 md:py-24 border-t border-neutral-100" aria-label="Related Products">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
        {products.map((rp) => {
          ...
          const hasSale =
            typeof rp.salePrice === 'number' && rp.salePrice > 0 && rp.salePrice < rp.price
          ...
          return (
            <div key={rp.id} className="group flex flex-col">
              <Link
                ...
                className="flex flex-1 flex-col focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-900 focus-visible:ring-offset-2"
              >
                {/* Image */}
                <div className="relative mb-3 aspect-[3/4] overflow-hidden bg-neutral-50">
                  {imageUrl ? (
                    <Image
                      src={imageUrl}
                      alt={img?.alt ?? rp.name}
                  ...
                  {hasSale && !isOutOfStock && (
                    <span className="pointer-events-none absolute top-3 left-3 z-10 bg-neutral-900 px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider text-white">
                      Sale
                    </span>
                  )}
                  ...
                  {/* Product link indicator — desktop only */}
                  <span className="absolute inset-x-0 bottom-0 z-10 hidden h-10 items-center justify-center bg-neutral-900/90 text-[9px] font-bold uppercase tracking-[0.15em] text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100 sm:flex">
                    View Product
                  </span>
                </div>
