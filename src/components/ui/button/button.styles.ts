...
const base =
  'relative inline-flex items-center justify-center gap-2 select-none font-medium transition-[transform,opacity] duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer'
...
const variants: Record<ButtonVariant, string> = {
  primary:
    'bg-button-primary text-button-alt overflow-hidden before:absolute before:inset-0 before:border-0 before:border-button-alt before:duration-100 before:ease-linear transition-all duration-300 hover:bg-button-alt hover:text-button-primary hover:shadow-button-primary before:hover:border-[25px] active:bg-button-alt active:text-button-primary active:shadow-button-primary before:active:border-[25px] cursor-pointer border border-button-primary',
  secondary:
    'bg-transparent text-content-primary border border-border-primary hover:bg-button-primary hover:text-button-alt hover:border-button-primary active:bg-button-primary active:text-button-alt active:border-button-primary transition-all duration-300 overflow-hidden before:absolute before:inset-0 before:border-0 before:border-button-alt before:duration-100 before:ease-linear before:hover:border-[25px] cursor-pointer',
  outlined:
    'border border-border-primary text-content-primary hover:bg-background-secondary active:bg-background-secondary cursor-pointer',
  ghost:
    'bg-transparent text-content-primary hover:bg-background-secondary active:bg-background-secondary cursor-pointer',
...
const iconMotion: Partial<Record<ButtonVariant, { left?: string; right?: string }>> = {
  back: { left: 'group-hover:-translate-x-0.5 group-active:-translate-x-0.5' },
  link: { right: 'group-hover:translate-x-0.5 group-active:translate-x-0.5' },
  cta: { right: 'group-hover:translate-x-0.5 group-active:translate-x-0.5' },
  text: { right: 'group-hover:translate-x-0.5 group-active:translate-x-0.5' },
}
